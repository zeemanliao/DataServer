...
GameStorage.prototype.run = function() {
		...
		for (let tableName in this.dataStruct) {
			...
			for (let _id in self.updates[tableName]) {
				...
				if (!checkData(data, self.datas[tableName][_id])) {
					...
					_update[tableName].push(data);
				}
			}
		}
		console.log(_update);
		for (let tableName in _update) {
			if (_update[tableName].length > 0) {
				self.db.put(tableName, _update[tableName], function(err){
					if (err)
						console.log(err);
				});
			}
	...
	this.interval = setTimeout(execRun, self.delay)
};
