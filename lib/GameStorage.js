...
function GameStorage(config) {
	...
	init(this);
	this.info = {runCount:0,updateCount:0,updateTotal:0,rollbackCount:0,rollbackTotal:0};
...
util.inherits(GameStorage, EventEmitter);
...
GameStorage.prototype.run = function() {
	...
	let execRun = function() {
		...
		self.emit(EVENT_BEFORE_RUN, tableName);
		let _updates = [];
...
GameStorage.prototype.rollback = function(tableName, _updates) {
	...
	this.info.rollbackCount ++;
	this.info.rollbackTotal += _updates.length;
	this.emit(EVENT_ROLLBACK, tableName, _updates);
};
	...
	let execRun = function() {
		...
		if (tableName) {
			db.get(tableName, {}, function(err, data){
				if (err) {
					return self.emit(EVENT_ERROR, err);
				} else {
